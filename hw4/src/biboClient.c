...
void signal_handler(int signo) {
    ...
    Request req;
    req.request = QUIT;
    write(client_fifo_fd, &req, sizeof(Request));
    close(client_fifo_fd);
    unlink(client_fifo);
...
int main(int argc, char *argv[]) {
    ...
    client_fifo_fd = open(client_fifo, O_RDONLY | O_CREAT, 0666);
    if (client_fifo_fd == -1) {
    ...
    if (bytes == sizeof(connection_status)) {
        ...
        {   
            ...
            client_fifo_fd = open(client_fifo, O_WRONLY | O_CREAT, 0666);
            if (client_fifo_fd == -1) {
                perror("open client fifo");
                close(server_fifo_fd);
                unlink(client_fifo);
